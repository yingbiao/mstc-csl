...
describe("Bibliography Chinese", function() {
    ...
    it("Bibliography::Booked viewed online", function() {
        ...
        expected = '<div class="csl-entry">何衛中：《牧養神的群羊》（香港：金燈台，2010），http//www.goldenlampstand.org/download/GL217A_Pastoral_TC.pdf（2014年9月8日參閱）。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Essay or chapter", function() {
        ...
        expected = '<div class="csl-entry">黄二冬：「天下一家」，《地球村的演變》（千百文編，厦門：漢陽出版社，2003），335–350。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Article in a lexicon or theological dictionary", function() {
        ...
        expected = '<div class="csl-entry">馮來平：「基督的寬容」，《世界神學辭典》（邱宋恩編，共四冊，上海：展徒出版社，1990），3：25–26。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Journal article", function() {
        ...
        expected = '<div class="csl-entry">楊品通：「神學的本質探討」，《教會神學期刊》14（2003年5月），19–24。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Magazine or newspaper article", function() {
        ...
        expected = '<div class="csl-entry">李約翰：「美國華僑習俗趣談」，《时代華僑周刊》（1957年10月30日），43–47。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Thesis or dissertation", function() {
        ...
        expected = '<div class="csl-entry">劉庸：「基督教教育在南非華人教會群體中的發展模式」（太平洋天國神學院：博士論文，2000）。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Webpage", function() {
        ...
        expected = '<div class="csl-entry">劉小楓：「歷史玩完了？」，《漢語基督教文化研究所》（2001年5月3日），http://www.iscs.org.hk/article18.htm（2002年8月5日參閱）。</div>';
        assert.equal(output, expected);
    });
    ...
    it("Bibliography::Journal article with translator", function() {
        ...
        var expected = '<div class="csl-entry">約翰•歐文（述寧译）：「約翰•歐文論牧師的責任」，《教會》70（2018年3月），https://www.churchchina.org/wp-content/uploads/ccpdf/070cc1803.pdf（2019年1月15日参阅），19。</div>';
        assert.equal(output, expected);
