...
describe("Citation Chinese", function() {
    ...
    it("Citation::Booked viewed online", function() {
        ...
        expected = '何衛中：《牧養神的群羊》（香港：金燈台，2010），http//www.goldenlampstand.org/download/GL217A_Pastoral_TC.pdf（2014年9月8日參閱），13。';
        assert.equal(output, expected);
    });
    ...
    it("Citation::Journal article", function() {
        ...
        expected = '楊品通：「神學的本質探討」，《教會神學期刊》14（2003年5月），21。';
        assert.equal(output, expected);
    });
    ...
    it("Citation::Magazine or newspaper article", function() {
        ...
        expected = '李約翰：「美國華僑習俗趣談」，《时代華僑周刊》（1957年10月30日），43。';
        assert.equal(output, expected);
    });
    ...
    it("Citation::Thesis or dissertation", function() {
        ...
        expected = '劉庸：「基督教教育在南非華人教會群體中的發展模式」（太平洋天國神學院：博士論文，2000），10。';
        assert.equal(output, expected);
    });
    ...
    it("Citation::Conferenec paper", function() {
        ...
        expected = '謝木水：「神學如何失去靈性？」（新加坡神學院益道論述會上發表之專文，新加坡，2010年10月4日），1。';
        assert.equal(output, expected);
    });
    ...
    it("Citation::Webpage", function() {
        ...
        expected = '劉小楓：「歷史玩完了？」，《漢語基督教文化研究所》（2001年5月3日），http://www.iscs.org.hk/article18.htm（2002年8月5日參閱）。';
        assert.equal(output, expected);
    });
    ...
    it("Citation::Journal article with translator", function() {
        ...
        expected = '約翰•歐文（述寧譯）：「約翰•歐文論牧師的責任」，《教會》70（2018年3月），https://www.churchchina.org/wp-content/uploads/ccpdf/070cc1803.pdf（2019年1月15日參閱），19。';
        assert.equal(output, expected);
    });
